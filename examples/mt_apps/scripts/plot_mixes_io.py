...
def parse_events(dataframe, tenant_id):
    ...
    return times, throughputs, fmqs


# Load data, add the completion time column and filter tenant congested traces
df = pd.read_csv("logs/io_mix.csv")
df = df[df["mix_name"] == "io"]
df["completion_time"] = df["end_t"] - df["start_t"]
df["io_throughput"] = df["pkt_size"] / (df["end_t"] - df["start_t"]) * 8
...
# Plot the results
ax[2].plot(app1_times, app1_occupations, label="Tenant 1", linewidth=LINE_WIDTH)
ax[2].plot(app2_times, app2_occupations, label="Tenant 2", linewidth=LINE_WIDTH)
ax[2].plot(app3_times, app3_occupations, label="Tenant 3", linewidth=LINE_WIDTH)
ax[2].plot(app4_times, app4_occupations, label="Tenant 4", linewidth=LINE_WIDTH)
ax[2].set_ylabel("IO throughput\n[Gbps]")
...
# Plot the results
ax[1].plot(app1_times, app1_occupations, label="Tenant 1", linewidth=LINE_WIDTH)
ax[1].plot(app2_times, app2_occupations, label="Tenant 2", linewidth=LINE_WIDTH)
ax[1].plot(app3_times, app3_occupations, label="Tenant 3", linewidth=LINE_WIDTH)
ax[1].plot(app4_times, app4_occupations, label="Tenant 4", linewidth=LINE_WIDTH)
ax[1].set_ylabel("IO throughput\n[Gbps]")
ax[0].set_ylabel("Fairness")
ax[0].get_xaxis().set_ticklabels([])
ax[0].plot(fairness_times, fairness, label="RR", color="red", linewidth=LINE_WIDTH)
ax[2].text(58000, 360, "WLBVT", fontweight="bold")
ax[1].text(60000, 340, "RR", fontweight="bold")
...
for index in range(len(max_RR)):
    ...
    if index == 2:
        ax[2].text((max_WLBVT[index] + max_RR[index]) / 2 - 5000, 370 - 1.4 * 70,
                   f"{(1 - time_WLBVT[index] / time_RR[index]) * 100:.2g}%",
                   fontweight="bold", color=colors[index], fontsize=19)
        ax[2].annotate("", xy=(max_WLBVT[index], 360 - 1.5 * 70),
                       xytext=(max_RR[index], 360 - 1.5 * 70),
                       arrowprops={"arrowstyle": '<->', "color": colors[index], "linewidth": 2})
    elif index == 3:
        ax[2].text((max_WLBVT[index] + max_RR[index]) / 2 - 370, 370 - 2 * 70,
                   f"{(1 - time_WLBVT[index] / time_RR[index]) * 100:.2g}%",
                   fontweight="bold", color=colors[index], fontsize=19)
        ax[2].annotate("", xy=(max_WLBVT[index], 360 - 2 * 70),
                       xytext=(max_RR[index], 360 - 2 * 70),
                       arrowprops={"arrowstyle": '<->', "color": colors[index], "linewidth": 2})
    else:
        ax[2].text((max_WLBVT[index]+max_RR[index])/2 - 1000, 370-(index % 3)*70, f"{(1-time_WLBVT[index]/time_RR[index])*100:.2g}%",
               fontweight="bold", color=colors[index], fontsize=19)
        ax[2].annotate("", xy=(max_WLBVT[index], 360 - (index % 3) * 70),
                       xytext=(max_RR[index], 360 - (index % 3) * 70),
                       arrowprops={"arrowstyle": '<->', "color": colors[index], "linewidth": 2})
...
ax[0].legend(loc="center", frameon=False, bbox_to_anchor=(0.59, 0.59),
...
ax[2].legend(handles=[Line2D([0], [0], color=colors[0], lw=4), Line2D([0], [0], color=colors[1], lw=4),
             ...
             labels=["IO read (V)", "IO write (V)", "IO read (C)", "IO write (C)"])
plt.subplots_adjust(hspace=0.13)
plt.savefig("./figures/hpu_mixes_IO_tput.pdf", bbox_inches='tight')
plt.show()
