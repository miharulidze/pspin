...
static void stcp_rx_handle_data(handler_args_t *handler_args, stcp_connection_t *connection,
                                tcp_header_t *tcp_hdr, size_t tcp_seg_size)
{
    ...
    if (rseqno < connection->next_to_receive) {
        STCP_DPRINTF("Packet falling before recv win. Dropping! "
                     "(rseqno: %lu; next_to_receive: %lu)\n",
    ...
    if (rseqno > max_seqno) {
        STCP_DPRINTF("Packet is falling on the right of the recv win! Dropping! "
                     "(rseqno: %lu; payload_size: %lu; max_seqno: %lu)\n",
    ...
    if (rseqno + payload_size > max_seqno) {
        STCP_DPRINTF("No buffer available! Dropping! "
                     "(rseqno: %lu; payload_size: %lu; max_seqno: %lu)\n",
                     rseqno, payload_size, max_seqno);
        return;
    }

    /* NOTE: we write anyway for now..
     * if later we find out that we don't have space in the heap then we will write again. */
    ...
    spin_cmd_t cmd;
#ifdef SPIN_SNITCH
    spin_host_dma(connection->buffer.host_ptr + buff_offset, (uint32_t)payload_ptr,
                  NIC_TO_HOST, payload_size, &cmd);
#else
    spin_dma_to_host(connection->buffer.host_ptr + buff_offset, (uint32_t)payload_ptr,
                     payload_size, 0, &cmd);
#endif

    /* OOO handling */
    amo_add(&connection->ooo_threads, 1);
    uint32_t res = ooo_range_push(&connection->ooo_man, handler_args->hpu_id,
                                  handler_args->cluster_id, stream_range);
    ...
    if (spin_lock_try_lock(&(connection->ooo_lock))) {
        uint32_t flushed_bytes = ooo_range_front_flush(&connection->ooo_man,
                                                       handler_args->hpu_id,
                                                       handler_args->cluster_id,
                                                       connection->next_to_receive);
        /*
        ...
        while (connection->ooo_threads==0 && (new_flushed_bytes>0 || connection->ooo_dirty))
        {
            ...
            new_flushed_bytes = ooo_range_front_flush(&connection->ooo_man, handler_args->hpu_id,
                                                      handler_args->cluster_id, connection->next_to_receive);
    ...
    while (connection->ooo_threads==0 &&
           (flushed_bytes>0 || connection->ooo_dirty) &&
           spin_lock_try_lock(&(connection->ooo_lock))) {
        connection->ooo_dirty = 0;
        flushed_bytes = ooo_range_front_flush(&connection->ooo_man,
                                              handler_args->hpu_id,
                                              handler_args->cluster_id,
                                              connection->next_to_receive);
        connection->next_to_receive += flushed_bytes;
             ...
             tcp_header_t *tcp_hdr, size_t tcp_seg_size)
{
    ...
    case STCP_ESTABLISHED:
        ...
        if (connection->fin_seen && connection->next_to_receive >= connection->fin_seqno) {
            ...
            STCP_DPRINTF("STCP_ESTABLISHED -> STCP_LAST_ACK (next_to_read: %lu; fin_seqno: %lu)",
                         connection->next_to_read, connection->fin_seqno);
        }
