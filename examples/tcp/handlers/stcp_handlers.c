...
{
/*
    void* l2_connection = task->handler_mem;
    spin_nic_dma(void *src, void* dst, uint32_t length, spin_cmd_t *handle)
*/
        ...
        spin_cmd_t cmd;
        spin_dma(args->task->handler_mem, cluster_state, args->task->handler_mem_size, SPIN_DMA_EXT2LOC, 0, &cmd);

        //printf("Handler mem: %p; cluster mem: %p; cluster_id: %lu; scratchpad[0]: %p %p\n", args->task->handler_mem, &(cluster_state->connection), args->cluster_id, &(args->task->scratchpad[0]), cluster_state);
        ...
        // TODO
#else
        connection->ooo_man.free_pool.mem_base_addr_start = (uint32_t) (((uint8_t*) &connection->ooo_man) + ((uint32_t) connection->ooo_man.free_pool.mem_base_addr_start));
        connection->ooo_man.free_pool.mem_base_addr_end = (uint32_t) (((uint8_t*) &connection->ooo_man) + ((uint32_t) connection->ooo_man.free_pool.mem_base_addr_end));
#endif
...
__handler__ void tcp_ph(handler_args_t *args)
{
    ...
    {
        //connection lookup
        //stcp_connection_t *connection = stcp_conn_lookup(stcp_state, tcp_hdr, sip); //(stcp_connection_t*) 0x1c300000; //
#ifdef CONNECTION_IN_L1
        cluster_state_t *cluster_state = ((cluster_state_t **)args->task->scratchpad)[args->task->home_cluster_id];
        stcp_connection_t *connection = &(cluster_state->connection);
        //printf("Connection %p\n", connection);
